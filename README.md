...
トップページには6つのエントリーがあり、いずれも利用できます。

| エントリー | 内容 |
| --- | --- |
| ドロップレットを作る | W/Oドロップレット・ゲルマイクロドロップ（GMD）の作製工程 |
| ソーティング・分注を行う | サンプルの解析・分離・プレート分注の工程 |
| ドロップレットから取り出す | 分注済みドロップレットからのサンプル解放。手作業とWork Stationの2方法 |
| 実験条件検討 | 実験目的・封入対象・培地・目標液滴・後工程から見解とPDFを作成 |
| 測定結果を確認する | フローサイトメーターの原理とプロットの見方をSTEP 1〜STEP 8で解説 |
| トラブルを解決する | 症状から確認項目と対応候補を探す |
...
| `sorting` ソーティング・プレート分注を行う | On-chip Droplet Selectorの分離・プレート分注 | 21 |
...
| `recovery_manual` | 手作業（Droplet Generator付属プラズマボール） | 5 |
| `recovery_workstation` | On-chip Droplet Work Station（Microdispenser＋Merge） | 11 |

基本工程データは全12コース・合計207工程です。On-chip Droplet Selectorを選択した場合は実行時に工程構成が変わるため、画面に表示される工程数はこの表と一致しません。

そのほかの収録データは、確認項目783件（工程直下693件、分岐内90件）、操作手順65件、製品71件、用語集319語・8分野、トラブル案内13項目です。
...
- トップページは6カードを3列×2段で表示し、デスクトップでは段ごとに同じ高さにそろえる
- 6カードすべてが利用可能になったため、利用可否のバッジは表示しない
