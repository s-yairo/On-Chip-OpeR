"""Streamlit route for the paid optional experiment condition advisor.

Paid Optional Plugin: Experiment Condition Advisor

This module is not part of the core experiment navigation workflow. It must
remain removable without affecting existing workflows, progress restoration,
product pages, result pages, or troubleshooting. Core code must reach this
module only through core.feature_registry.
"""
...
PLUGIN_ID = "experiment_condition_advisor"
PLUGIN_VERSION = "0.1.0-prototype"
PLUGIN_DATA_SCHEMA_VERSION = 1
REPORT_STATE_KEY = "eca_generated_opinion"


def _load_sources(plugin_dir: Path) -> list[dict[str, Any]]:
    path = plugin_dir / "data" / "approved_sources.json"
    try:
...
def _render_plugin_css() -> None:
    st.markdown(
        ...
        .eca-plugin-label{display:inline-block;border:1px solid #8a63d2;background:#f7f2ff;color:#5a3f9b;border-radius:999px;padding:.22rem .58rem;font-size:.78rem;font-weight:800;margin-bottom:.55rem}
...
def render(*, context: dict[str, Any], feature_status: Any) -> None:
    _render_plugin_css()
    plugin_dir = Path(__file__).parent
    navigation_version = str(context.get("navigation_version", ""))
    product_matcher = context.get("find_droplet_product_candidates")
    catalog = context.get("product_catalog", {})
    approved_sources = _load_sources(plugin_dir)

    st.title("実験条件検討")
    st.markdown(
        f"""
        <div class="eca-hero">
          <div class="eca-plugin-label">有料オプション｜{feature_status.badge}</div>
    ...
    payload = {
        ...
        "plugin_data_schema_version": PLUGIN_DATA_SCHEMA_VERSION,
    ...
    if generated:
        ...
        if pdf_is_available():
            ...
            st.warning("PDF機能は任意依存関係です。optional_requirements/experiment_condition_advisor.txt をインストールしてください。")
